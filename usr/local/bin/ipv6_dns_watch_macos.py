...
Shutdown cleanup is handled by a separate launchd one-shot job
(au.hosteng.ipv6-dns-shutdown.plist) which runs ipv6_dns_sync.py
with --shutdown when the system shuts down. This is more reliable
than SIGTERM handling in a KeepAlive daemon, as launchd does not
guarantee SIGTERM delivery on subsequent shutdowns.
